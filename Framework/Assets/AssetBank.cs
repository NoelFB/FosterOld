...
namespace Foster.Framework
{
    ...
    public abstract class AssetBank
    {
        ...
        private class Resource
        {
            ...
            public T? Reload<T>() where T : Asset
            {
                T? target = null;
                ...
                if (Bank.GetAssetStream(Guid, out var stream, out var metadata) && stream != null)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        if (Asset == null)
                            Asset = new WeakReference<Asset>(target);
                        else
        ...
        /// </summary>
        public Guid? GuidOf<T>(string name) where T : Asset
        {
        ...
        /// Gets a given Asset
        /// </summary>
        public T? Get<T>(Guid guid) where T : Asset
        {
            if (byGuid.TryGetValue(guid, out var resource))
                return resource.Get<T>();
            return null;
        ...
        /// </summary>
        public T? Reload<T>(string name) where T : Asset
        {
            if (byName.TryGetValue(typeof(T), out var dictionary) && dictionary.TryGetValue(name, out var resource))
        ...
        /// Gets a given Asset and forces a Reload
        /// </summary>
        public T? Reload<T>(Guid guid) where T : Asset
        {
            if (byGuid.TryGetValue(guid, out var resource))
                return resource.Reload<T>();
            return null;
        ...
        /// </summary>
        public IEnumerable<T> Each<T>(string? prefix = null) where T : Asset
        {
        ...
        /// </summary>
        public AssetHandle<T> Handle<T>(string name) where T : Asset
        {
        ...
        /// Makes an AssetHandle to an Asset
        /// </summary>
        public AssetHandle<T> Handle<T>(Guid guid) where T : Asset
        {
        ...
        /// </summary>
        public void Dispose<T>(string name) where T : Asset
        {
            if (byName.TryGetValue(typeof(T), out var dictionary) && dictionary.TryGetValue(name, out var resource))
                resource.Dispose();
        }
        ...
        /// Disposes the Asset, forcing a Reload the next time it's requested
        /// </summary>
        public void Dispose<T>(Guid guid) where T : Asset
        {
            if (byGuid.TryGetValue(guid, out var resource))
                resource.Dispose();
        }
